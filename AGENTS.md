...
- **Attacks** (`internal/attacker/`) — Vegeta-orchestrated rate attacks. Each attack carries `experiment_id`, `meta_trace_id`, and `workflow_label`; `meta_trace_id` and `workflow_label` are injected into W3C Baggage (`meta-trace-id`, `atropos.workflow`) so the atropos SDK can tag recorded cache entries. `POST /attacks` returns **201**. Final metrics: `GET /attacks/{id}/result` (manteion's `AttackResultInfo` shape — 404 until the attack finalizes). List filters: `?status=running&experiment_id=X`. Per-attack `/stats` and `/result` endpoints.
...
6. **(Optional) Concurrent precision attack.** `POST /attacks` with `experiment_id`, `meta_trace_id`, and `workflow_label` matching the workflow run (these correlate the attack to the phase). Used for "spike one endpoint while the broad workflow runs." Poll `GET /attacks/{id}/result` for final metrics.
